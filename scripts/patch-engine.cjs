...
function run() {
    let js = fs.readFileSync('public/js/mappingEngine.js', 'utf8');
    ...
    if (targetRegex.test(js)) {
        js = js.replace(targetRegex, overshootCode);
        fs.writeFileSync('public/js/mappingEngine.js', js);
        console.log("Successfully injected the Overshoot Mask logic into mappingEngine.js.");
    } else {
        if (js.includes('// 7. Overshoot Mask (Yellow)')) {
