...
function run() {
    ...
    if (html.includes(targetChunk)) {
        html = html.replace(targetChunk, overshootCode);
        ...
        if (html.includes('// 7. Overshoot Mask (Yellow)')) {
            ...
            console.log("Target chunk not found. Make sure the structure matches.");
        }
    }
}
